...
class OpenMax:
    def __init__(self, activations_path, tail_size=20, weibull_params_path='./weibulls.pkl', alpha=0.3):
        """
        初始化OpenMax类。
    ...
    def _fit_weibulls(self):
        """
        为每个已知类别（1-23）拟合Weibull分布，并保存Weibull模型参数。
        """
        for cls, feats in self.activations.items():
            if cls == 0:
                # 跳过 'Unknown' 类
                continue
            class_label = cls  # 类别标签保持为1-23
    ...
    def compute_adjusted_scores(self, features, logits):
        ...
        for i, feat in enumerate(features):
            ...
            if cls == 0:
                ...
                # 无效的类别，保持原有概率分布
                # print(f"样本 {i}: 预测类别 {cls} 超出范围 [0,23]，保持原有概率分布。")
                adjusted_score = probs[i]
    ...
    # 在 OpenMax 类中添加以下方法
def generate_unknown_samples(self, generator, num_samples=1000, device='cuda'):
    """
    生成用于 OpenMax 的未知类样本
    
    Args:
        generator (nn.Module): 训练好的生成器
        num_samples (int): 需要生成的样本数量
        device (str): 设备类型
    
    Returns:
        np.ndarray: 生成的假样本特征向量
    """
    generator.eval()
    
    # 生成噪声（正确维度）
    noise = torch.randn(num_samples, generator.nz, device=device)  # 2D张量 [batch, nz]
    
    # 生成未知类标签（0号类别）
    fake_labels = torch.zeros(num_samples, dtype=torch.long, device=device)  # 1D类别索引
    ...
    # 提取特征
    _, features = model(fake_images)  # 假设 model 是已定义的特征提取模型
