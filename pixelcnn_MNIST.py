...
class PixelCNN(nn.Module):
    def __init__(self, nr_filters=160, nr_resnet=5, nr_logistic_mix=10, disable_third=False, dropout_p=0.5, n_channel=1,
                 image_wh=28):
        ...
        # 移除下采样或者将下采样stride改为1，以保持分辨率不变
        # 下面是去掉下采样层的示例，不做下采样，保持分辨率为28x28
        ...
        self.conv_final = WN_Conv2d(nr_filters, n_channel * nr_logistic_mix, kernel_size=1)
...
def discretized_mix_logistic_loss_c1(x, l, sum_all=True):
    # 此时 l 的分辨率应与 x 一致，如28x28
    B, C, H, W = x.size()
    nr_mix = l.size(1) // 3

    # 确保 l 的分辨率与 x 相同，否则需要相应修改网络结构
    if l.size(2) != H or l.size(3) != W:
        raise ValueError(f"L and X have different spatial sizes: X({H}x{W}), L({l.size(2)}x{l.size(3)})")
    ...
    if sum_all:
        return -torch.sum(log_probs) / (B * H * W)
    else:
...
#     model = PixelCNN(nr_filters=160, nr_resnet=5, nr_logistic_mix=10, disable_third=False, dropout_p=0.5, n_channel=1,
#                      image_wh=28).to(device)
#     input_tensor = torch.rand(64, 1, 28, 28).to(device)
#     output = model(input_tensor)
#     print(f"Output shape: {output.shape}")  # 输出应为[64, 30, 28, 28]，因为 nr_logistic_mix=10, n_channel=1 => 3*10=30
#
#     # 测试loss
#     loss = discretized_mix_logistic_loss_c1(input_tensor, output)
...
if __name__ == '__main__':
    ...
    l = model(x)
    loss = discretized_mix_logistic_loss_c1(x, l)
    assert_nan(loss)
    print(loss.item())
